      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {selectedShapeObj?.type === "polygon" && (
          <div>
            ...
            <input
              type="text"
              value={selectedShapeObj.name}
              onChange={(e) => onNameUpdate(selectedShapeObj.id, e.target.value)}
              placeholder="e.g. Dining Area"
              className="w-full px-3 py-2 border border-gray-300 rounded font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        )}
        <div>
